...
function validateInput(inputValue, errorElement, errorMessage, regex, inputElement) {
    if (inputValue === '') {
        ...
        if (regex == usernameRegex) errorElement.textContent = 'no spaces, 3-15 characters, must start with a letter';
        errorElement.style.display = 'block';
        inputElement.classList.remove('valid-input');
        inputElement.classList.add('invalid-input');
    } else {
...
const inputs = [
    { input: document.getElementById('username'), errorElement: usernameErrorEl, message: 'Username is required.', regex:usernameRegex},
